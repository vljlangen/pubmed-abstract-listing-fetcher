...
// Prefer IPv4 first; avoids some undici/fetch failures on networks with broken IPv6.
...
const NCBI_CONTACT_EMAIL = process.env.NCBI_CONTACT_EMAIL || "your-email@example.com";
const NCBI_API_KEY = process.env.NCBI_API_KEY || ""; // optional: export NCBI_API_KEY in your environment
const MAX_RETRIES = 5;
const MIN_REQUEST_INTERVAL_MS = NCBI_API_KEY ? 120 : 400;
...
async function main() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      if (!pmid) {
        console.log("  No PubMed match found.");
        blocks.push({
          original: refLine,
          status: "not-found",
          abstract: ""
        ...
        if (absText) {
          ...
          console.log("  No abstract text returned.");
          blocks.push({
            original: refLine,
            status: "not-found",
            abstract: ""
      ...
      console.error("  Error while querying PubMed:", err.message, err.cause ? `(${err.cause.message || err.cause})` : "");
      const errMsg = String(err && err.message ? err.message : err).slice(0, 800);
      blocks.push({
        original: refLine,
        status: "error",
        abstract: "",
